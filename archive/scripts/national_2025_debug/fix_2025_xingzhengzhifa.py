...
for mod in ['changshi', 'yanyu', 'shuliang', 'panduan', 'ziliao']:
    path = f'src/data/xingce/{mod}/national_2025_xingzhengzhifa.json'
    try:
...
for q in all_questions:
    ...
    if num not in seen:
        ...
        # 如果重复，保留选项更多的那个
        existing_opts = len(seen[num].get('options', []))
        current_opts = len(q.get('options', []))
        if current_opts > existing_opts:
            # 替换
            idx = unique_questions.index(seen[num])
            unique_questions[idx] = q
            seen[num] = q
            print(f"  题号 {num} 重复，保留选项更多的版本 ({current_opts} > {existing_opts})")
...
# 标准题号范围（行政执法 135 题，但实际 PDF 可能只有 130 题）
# 先按实际情况分配
ranges = {
    'changshi': (1, 20),
    'yanyu': (21, 60),
...
for mod in new_modules:
    ...
    if actual > 0:
        nums = sorted([int(q['id'].split('-')[-1]) for q in new_modules[mod]])
        print(f"  {status} {mod}: {actual} 题 (题号 {nums[0]}-{nums[-1]})")
    else:
